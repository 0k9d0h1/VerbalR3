...
def reranker_collate_fn(batched_data_list: list[list[dict]]) -> dict:
    """
    Collate a batch of sample dicts into batched tensors and arrays.
    ...
    for data_list in batched_data_list:
        for data in data_list:
            for key, val in data.items():
                if isinstance(val, torch.Tensor):
                    tensors[key].append(val)
...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.max_score = config.get("reranker_max_score", 5)
    ...
    def __getitem__(self, item):
        ...
        #         if "ms_marco" in row_dict["data_source"]:
        #             passages = row_dict["extra_info"].pop("passages", None)
        #             query = row_dict.pop("prompt")[0]["content"]
        #             is_selected = passages["is_selected"]
        #             passage_text = passages["passage_text"]

        #             data_for_query = [copy.deepcopy(row_dict) for _ in range(len(passage_text))]
        #             for idx, (text, selected) in enumerate(zip(passage_text, is_selected, strict=False)):
        ...
        #                 data_for_query[idx]["input_ids"] = input_ids[0]
        #                 data_for_query[idx]["attention_mask"] = attention_mask[0]
        #                 data_for_query[idx]["position_ids"] = position_ids[0]
        #                 data_for_query[idx]["raw_prompt_ids"] = raw_prompt_ids
        #                 data_for_query[idx]["reward_model"] = {"style": "rule", "ground_truth": selected}
        #                 data_for_query[idx]["extra_info"]["answer"] = selected
        #                 data_for_query[idx]["extra_info"]["max_score"] = self.max_score

        #                 if self.return_raw_chat:
        #                     data_for_query[idx]["raw_prompt"] = messages

        #                 # get prompts with chat template
        #                 if self.return_full_prompt:
        #                     data_for_query[idx]["full_prompts"] = raw_prompt  # array of strings
