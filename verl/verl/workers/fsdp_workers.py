...
class RewardModel(nn.Module):
    def __init__(self, model_name):
        super().__init__()
        # Load the base transformer's configuration and model
        self.config = AutoConfig.from_pretrained(model_name, trust_remote_code=True)
        self.base_model = AutoModel.from_pretrained(model_name, config=self.config, trust_remote_code=True)

        # Define the regression head exactly as in your training script
        self.regression_head = nn.Sequential(
            nn.Linear(self.config.hidden_size, 512), nn.ReLU(), nn.Dropout(0.1), nn.Linear(512, 1), nn.Sigmoid()
        )

    def forward(self, input_ids, attention_mask):
        # The forward pass doesn't need to be run, but it must be defined
        # for the class to be complete.
    ...
    def save_pretrained(self, save_directory):
        """
        A custom save method to handle the two parts of the model.
        """
        print(f"Saving model to {save_directory}")
        # Save the base model and its config, which will create the safetensors and config.json
        self.base_model.save_pretrained(save_directory, safe_serialization=True)
        # Save the regression head's state dict separately
        torch.save(self.regression_head.state_dict(), os.path.join(save_directory, "regression_head.pth"))
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if config.reranker_training:
                reward_module = RewardModel(local_path)
                head_weights_path = os.path.join(local_path, "regression_head.pth")
                reward_module.regression_head.load_state_dict(torch.load(head_weights_path))
            else:
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.config.reranker_training:
                    output = self.reward_module(input_ids=input_ids, attention_mask=attention_mask)
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        if self._do_switch_chat_template:
            ...
            if self.config.reranker_training:
                rm_input_ids = data.batch["reward_model_input_ids"]
                rm_attention_mask = data.batch["reward_model_attention_mask"]
                rm_position_ids = data.batch["reward_model_position_ids"]
            else:
