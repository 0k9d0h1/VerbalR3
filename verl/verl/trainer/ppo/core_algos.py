...
def compute_token_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    config: Optional[AlgoConfig] = None,
    phrase_token_lengths: Optional[list[list[int]]] = None,
    advantage_penalty: Optional[list[list[float]]] = None,
    prompt_length: Optional[list[int]] = None,
    input_ids: Optional[list[list[int]]] = None,
    ):
    scores = token_level_rewards.sum(dim=-1)
    response_mask_len = response_mask.shape[1]
    ...
    if phrase_token_lengths is not None and advantage_penalty is not None:
        token_level_penalties = []
        for i in range(len(scores)):
            phrase_lengths = phrase_token_lengths[i]
            penalties = advantage_penalty[i]
            if len(phrase_lengths) - 1 != len(penalties):
                print(f"Error: phrase lengths: {phrase_lengths}, penalties: {penalties}")
                # print(f"Decoded input ids: {tokenizer.decode(input_ids[i], skip_special_tokens=True)}")

                # When error occurs because of the planner generating information blocks, we set penalties to zero.
                token_level_penalties.append(torch.zeros(response_mask_len, device=scores.device))
            else:
                token_level_penalty = []
                for penalty, length in zip(penalties, phrase_lengths[:-1]):
                    token_level_penalty.extend([penalty] * length)

                remain_length = response_mask_len - len(token_level_penalty)
                if remain_length > 0:
                    token_level_penalty.extend([0.0] * remain_length)
                token_level_penalty_tensor = torch.tensor(token_level_penalty, device=scores.device)

                token_level_penalties.append(token_level_penalty_tensor)
        token_level_penalties_tensor = torch.stack(token_level_penalties, dim=0)
    else:
        token_level_penalties_tensor = torch.zeros_like(token_level_rewards)
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            if norm_adv_by_std_in_grpo:
                ...
                scores[i] = scores[i] - id2mean[index[i]]
        scores = scores.unsqueeze(-1) * response_mask
        scores = scores + token_level_penalties_tensor * response_mask
