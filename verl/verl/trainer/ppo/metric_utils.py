...
def calculate_zero_variance_ratio(
    sequence_score: torch.Tensor,
    repeat_times: int
) -> float:
    """
    Calculates the portion of chunks that have zero variance (all elements are identical)
    compared to the total number of chunks.

    Args:
        sequence_score: A 1D tensor of shape [batch_size].
        repeat_times: The number of elements per chunk.

    Returns:
        The ratio (0.0 to 1.0) of chunks with zero standard deviation.
    """
    
    # --- 1. Validate and Reshape ---
    if sequence_score.numel() == 0:
        return 0.0
        
    if sequence_score.shape[0] % repeat_times != 0:
        raise ValueError("Total number of scores is not divisible by repeat_times.")
        
    # Reshape to (num_chunks, repeat_times)
    num_chunks = sequence_score.shape[0] // repeat_times
    chunked_scores = sequence_score.view(num_chunks, repeat_times)

    # --- 2. Calculate Std and Ratio (Vectorized) ---
    
    # Calculate std deviation for every chunk at once.
    # unbiased=False is used so that if repeat_times == 1, it returns 0.0 (population std)
    # instead of NaN (sample std requires N > 1).
    stds = torch.std(chunked_scores, dim=1, unbiased=False)
    
    # Create a boolean mask where std is exactly 0
    zero_variance_mask = (stds == 0)
    
    # Calculate the mean of the float mask (True -> 1.0, False -> 0.0)
    # This gives the ratio: count_zero_var / total_chunks
    ratio = zero_variance_mask.float().mean().item()
    
    return ratio

def calculate_outcome_reward_mean(sequence_score):
    outcome_reward = []
    sequence_score_list = sequence_score.tolist()
    for score in sequence_score_list:
        if score >= 0.72 and score <= 0.8:
            outcome_reward.append(0.8)
        elif score >= 0.9:
            outcome_reward.append(1.0)
        else:
            outcome_reward.append(score)
    
    return sum(outcome_reward) / len(outcome_reward) if len(outcome_reward) != 0 else 0
    

def compute_data_metrics(batch: DataProto, repeat_times: int, use_critic: bool = True) -> dict[str, Any]:
    """
    ...
    zero_variance_ratio = calculate_zero_variance_ratio(sequence_score, repeat_times)
    outcome_reward_mean = calculate_outcome_reward_mean(sequence_score)
    ...
    metrics = {
        ...
        "critic/score/zero_variance_ratio": zero_variance_ratio,
        "critic/score/outcome_reward_mean": outcome_reward_mean,
        # reward
    ...
    if "refine_ratio" in batch.meta_info:
        metrics["env/refine_ratio"] = float(batch.meta_info["refine_ratio"])

    if "num_expanded" in batch.meta_info:
        metrics["env/num_expanded"] = float(batch.meta_info["num_expanded"])
